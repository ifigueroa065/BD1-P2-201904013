...
BEGIN
  ...
  IF LENGTH(telefono_in) != 8 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El teléfono debe tener 8 dígitos sin código de área';
    RETURN FALSE;
  END IF;
...
BEGIN
  ...
  IF nombre_in REGEXP '^[A-Za-z ]+$' = 0 THEN
...
BEGIN
  ...
  IF docente_existente > 0 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Este docente ya ha sido registrado en el sistema';
    RETURN FALSE;
  END IF;
...
BEGIN
  ...
  IF creditos_necesarios_in < 0 OR creditos_otorga_in < 0 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Los créditos necesarios y otorgados deben ser enteros positivos';
    RETURN FALSE;
  END IF;

  -- Validar que la carrera pertenezca a un área común o tenga un identificador válido
  IF carrera_in < 0 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El identificador de la carrera no es válido';
    RETURN FALSE;
  END IF;
...
BEGIN
  ...
  IF NOT (ciclo_in IN ('1S', '2S', 'VJ', 'VD')) THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El ciclo especificado no es válido';
    RETURN FALSE;
  END IF;

  -- Validar que el cupo máximo sea un número entero positivo
  IF cupo_maximo_in < 0 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El cupo máximo debe ser un número entero positivo';
    RETURN FALSE;
  END IF;

  -- Validar que la sección sea una letra y guardarla en mayúscula
  IF LENGTH(seccion_in) <> 1 OR NOT seccion_in REGEXP '^[A-Za-z]$' THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'La sección debe ser una única letra en mayúscula';
    RETURN FALSE;
  END IF;
...
BEGIN
  ...
  IF curso_habilitado_existente = 0 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El curso habilitado con el ID especificado no existe';
    RETURN FALSE;
  END IF;

  -- Validar que el día esté dentro del dominio [1,7]
  IF dia_in < 1 OR dia_in > 7 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El día debe estar dentro del dominio [1,7]';
    RETURN FALSE;
  END IF;

  -- Validar que el horario tenga un formato válido
  IF horario_in NOT REGEXP '^[0-2]?[0-9]:[0-5][0-9]-[0-2]?[0-9]:[0-5][0-9]$' THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El formato del horario no es válido';
    RETURN FALSE;
  END IF;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM AsignacionCurso ac
    ...
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El estudiante ya está asignado a este curso o a otra sección del mismo';
    RETURN FALSE;
  END IF;
  ...
  IF creditos_otorga > 0 AND creditos_necesarios > 0 THEN
    ...
    AND (SELECT CarreraID FROM Curso WHERE Codigo = codigo_curso_in) != carrera_id THEN
      SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El estudiante no cumple con los requisitos del curso';
      RETURN FALSE;
    END IF;
  END IF;
  ...
  IF cupo_actual >= cupo_maximo THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'La sección del curso ha alcanzado el cupo máximo';
    RETURN FALSE;
  END IF;
...
BEGIN
  ...
  IF estudiante_asignado = 0 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El estudiante no está asignado a este curso o sección';
    RETURN FALSE;
  END IF;
...
BEGIN
  ...
  IF nota_in < 0 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'La nota debe ser positiva';
    RETURN FALSE;
  END IF;
...
BEGIN
  ...
  IF total_notas_ingresadas < total_estudiantes THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'No se han ingresado las notas de todos los estudiantes asignados al curso';
    RETURN FALSE;
  END IF;
